...
print('<h1 style="color: #0000CD;" >Output</h1>')
...
#gpgcheck=0
#'''
        #output = sp.getoutput("setenforce 0")
        #output = sp.getoutput("touch dock.repo")
        #output = sp.getoutput("chown apache dock.repo") 
        #output = sp.getoutput("chmod +rwx dock.repo")
        #f = open("dock.repo","w")
        #f.write(dock_yum)
        #f.close()        
        #output = sp.getoutput("cp dock.repo /etc/yum.repos.d/dock.repo")
        #output = sp.getoutput(" yum install docker-ce --nobest -y")
        #output = sp.getoutput(" systemctl start docker")
        #print("<span><br>#### Started Docker Services ####</span><br>")
        #print("<pre>{}</pre>".format(output))
elif choice == 2:
        output = sp.getoutput('systemctl enable docker')
        print("<span><br>#### Made services permanent ####</span><br>")
        print("<pre>{}</pre>".format(output))
elif choice == 3:
        output = sp.getoutput('systemctl status docker')
        ...
        if pull_tag == None:
                pull_tag = "latest"
        output = sp.getoutput('docker pull {}:{}'.format(pull_image,pull_tag))
        print("<span><br>#### Image Pulling Done!!! ####</span><br>")
        print("<pre>{}</pre>".format(output))
elif choice == 8:
        if launch_tag == None:
                launch_tag = "latest"
        output = sp.getoutput('docker run -dit --name {} {}:{}'.format(launch_name,launch_image,launch_tag))
        print("<span><br>#### Launched a container named -> {} ####</span><br>".format(launch_name))
        print("<pre>{}</pre>".format(output))
elif choice == 9:
        if create_img_tag == None:
                create_img_tag = "latest"
        output = sp.getoutput('docker commit {} {}:{}'.format(create_img_name,create_img_image,create_img_tag))
        print("<span><br>#### Created a custom image named -> {} ####</span><br>".format(create_img_name))
        print("<pre>{}</pre>".format(output))
elif choice == 10:
        output = sp.getoutput('docker start {}'.format(start_name))
        print('<span><br>#### Started container named -> {} ####</span><br>'.format(start_name))
elif choice == 11:
        output = sp.getoutput('docker stop {}'.format(stop_name))
        print('<span><br>#### Stopped container named -> {} ####</span><br>'.format(stop_name))
elif choice == 12:
        if remove_image == "all":
                output = sp.getoutput('docker rmi `docker images -a -q`')
                print("<span><br>#### Removed all images!!! ####</span><br>")
        else:
                if remove_tag == None:
                        remove_tag = "latest"
                output = sp.getoutput('docker rmi {}:{}'.format(remove_image,remove_tag))
                print('<span><br>#### Removed image named -> {} ####</span><br>'.format(remove_image))
elif choice == 13:
        if remove_name == "all":
                output = sp.getoutput('docker rm `docker ps -a -q`')
                print("<span><br>#### Removed all containers!!! ####</span><br>")
        else:
                output = sp.getoutput('docker rm {}'.format(remove_name))
